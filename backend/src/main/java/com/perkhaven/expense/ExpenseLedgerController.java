...
public class ExpenseLedgerController {
 ...
 @GetMapping @PreAuthorize("isAuthenticated()") @Transactional(readOnly=true) public Map<String,Object> list(){return Map.of("expenses",expenses.findAllByOrderByTransactionDateDescIdDesc().stream().map(Response::from).toList(),"categories",categories.findAll().stream().map(CategoryResponse::from).toList());}
 @PostMapping(consumes=MediaType.MULTIPART_FORM_DATA_VALUE) @PreAuthorize("isAuthenticated()") @Transactional public Map<String,Object> create(@RequestParam long categoryId,@RequestParam BigDecimal amount,@RequestParam LocalDate transactionDate,@RequestParam String personPaidStaffNo,@RequestParam String settlingMethod,@RequestParam(defaultValue="") String remarks,@RequestPart("evidence") MultipartFile evidence)throws IOException{
  if(evidence.isEmpty()||amount.signum()<=0)throw new IllegalArgumentException("Valid amount and evidence are required."); var category=categories.findById(categoryId).orElseThrow(()->new NotFoundException("Expense category not found.")); var member=staff.findByStaffNoIgnoreCase(personPaidStaffNo).orElseThrow(()->new NotFoundException("Staff member not found.")); var stored=storage.store("expense-evidence",evidence); var seq=sequences.findForUpdate("EXPENSE").orElseThrow(); var expense=expenses.save(new Expense("E-%04d-%04d".formatted(transactionDate.getYear(),seq.takeNextValue()),category,amount,transactionDate,member.getStaffNo(),member.getFirstName()+" "+member.getLastName(),settlingMethod,stored.key(),stored.originalName(),stored.contentType(),remarks)); audit.record("CREATE","EXPENSE",expense.getTransactionId(),null); return Map.of("expense",Response.from(expense)); }
 ...
 @GetMapping("/evidence") @PreAuthorize("isAuthenticated()") public ResponseEntity<Resource> evidence(@RequestParam String transactionId){var e=expenses.findByTransactionId(transactionId).orElseThrow(()->new NotFoundException("Expense not found."));return ResponseEntity.ok().contentType(MediaType.parseMediaType(e.getEvidenceContentType())).header(HttpHeaders.CONTENT_DISPOSITION,"inline; filename=\""+e.getEvidenceName().replace("\"","")+"\"").body(storage.load(e.getEvidenceKey()));}
 @GetMapping("/categories") @PreAuthorize("isAuthenticated()") public Map<String,Object> categories(){return Map.of("categories",categories.findAll().stream().map(CategoryResponse::from).toList());}
